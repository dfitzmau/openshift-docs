...
spec:
  ...
  to:
    ...
    name: service-name
----
* `spec.path`:: Specifies the path attribute is the only added attribute for a path-based route.

[NOTE]
====
Path-based routing is not available when using passthrough TLS. This occurs because the router does not terminate TLS and cannot read the contents of the request.
====
