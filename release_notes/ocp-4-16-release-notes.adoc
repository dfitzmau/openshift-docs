...
=== RHBA-2024:10973 - {product-title} {product-version}.26 bug fix and security update
...
[id="ocp-4-16-27-known-issues_{context}"]
==== Known issues

* If you plan on using the NUMA Resources Operator in your {product-title} cluster, https://issues.redhat.com/browse/OCPBUGS-45983 asking SME.

[id="ocp-4-16-27-bug-fixes_{context}"]
==== Bug fixes

// SME review needed
* Previously, when `openshift-sdn` pods  were deployed during the {product-title} {product-version} upgrading process, the Open vSwitch (OVS) storage table was cleared. Ports for existing pods had to be re-created and this caused disruption to numerous services. With this release, a fix ensures if a pod becomes disconnected from the network during the cluster upgrade operation, services continue to run as expected without any disruptions. (link:https://issues.redhat.com/browse/OCPBUGS-45806[*OCPBUGS-45806*])

* Previously, you could not remove a finally pipeline task from the *edit Pipeline* form if you created a pipeline with only one finally task. With this release, you can remove the finally task from the *edit Pipeline* form and the issue is resolved. (link:https://issues.redhat.com/browse/OCPBUGS-45229[*OCPBUGS-45229*])ssss
